...
import os,json,shutil,sys,re,hou,glob,re
import subprocess
import atexit
import toolutils
...
def collectdir(node):
    collectpath=hou.getenv('MS_COLLECT')
    val=''
    if collectpath != None:
        data=collectpath
        if os.path.isdir(data):
            val="$MS_COLLECT/"+"`chs("+'"category"'+")`"+"/`chs("+'"name"'+")`"
    else:
...
def start_bridge():
    ...
    try: 
        ...
        if os.path.isfile(path):
            ...
            if platform=='Linux':
                launch=[os.path.normpath((path+"/Bridge.AppImage").replace('\\','/')),"--no-sandbox"]
            else:
                launch=[os.path.normpath((path+"/Bridge.exe").replace('\\','/'))]

        if os.path.isfile(launch[0]):
            try:
                with open(os.devnull, 'w') as fp:
                    proc = subprocess.Popen(launch, shell=False , stdout=fp, start_new_session=True)
                    child_pid = proc.pid
...
def cleannodeparm(node):
    try:
        ...
        parms=parms.findFolder('content')
...
def convertrat(node,tex,tconvert=0,acesuse=0):
    ...
    if node.parm('resolution') is not None:
        ...
        sres='_'+str(node.parm('resolution').eval())+'_'        
...
def collect(node):           
    ...
    assetfolder=os.path.dirname(jsonfile)
    jsondir=work_dir
    assetjs=os.path.basename(jsonfile)
    ...
    try:
        shutil.copytree(assetfolder,work_dir, dirs_exist_ok=True)  ###full collect directory
    except:
        pass 
    newjs=work_dir+'/'+assetjs
    parms=node.type()       
    parms=parms.definition()
    parms=parms.parmTemplateGroup()
    parms=parms.findFolder('content')
    ...
    for p in parms:
        ...
        if str(cont)!="" and type(cont).__name__=='str':
            if not 'UDIM' in cont:
                ...
                if get_platform()=='Windows':
                    newcont=newcont.replace(os.sep,'/')
                    job=job.replace('\'','/')
...
#                print('Content path do not exists!!!')
    ...
    if categorytype(node)!=2:
        ...
        if context=='Sop':
            ...
            bgeos = [dir+'/'+f for (dir, subdirs, fs) in os.walk(basepath) for f in fs if f.endswith(".usd")]
        for bgeo in bgeos:
    ...
    if node.parm('converted').eval()!=0 and shader is not None:
        node.parm('convertshader').pressButton()
    node.parm('json').set(newjs) ###replace jsonfile
...
def bakegeo(node):
    ...
    if context == 'Sop':
        ...
        filepath=bake_node.parm('lopoutput').eval()
    job=hou.getenv('JOB')
    ...
    if context == 'Sop':
        ...
        file=node.changeNodeType("sublayer",keep_parms=False) ###Change to filenode
        file.parm("filepath1").set(filepath)
...
def load3djson(node,jsondata,file):
    tags=jsondata['tags']
    texsizedata=getsizejs(jsondata,type=0) #for surface
    node.parmTuple('size').set(texsizedata)
...
###PREVIEW
    ...
    for p in previews:
        ...
        if os.path.isfile(pfile):
            ...
            break
    if get_platform()=='Windows':
        preview_file=preview_file.replace(os.sep,'/')
...
def getsizejs(jsondata,type=1):
    ...
    if type==1:
        try:
            meta=jsondata['meta']
            for m in meta:
                if m['name'].lower()=='length':
                    x=float(m['value'].replace('m',''))
                if m['name'].lower()=='width':
                    z=float(m['value'].replace('m',''))
                if m['name'].lower()=='height':
        ...
        try:
            meta=jsondata['meta']
            for m in meta:
                if m['name'].lower() =='scan area':
                    val=re.findall(r'\d+',m.get('value','1x1'))
                    x=float(val[0])
                    z=float(val[1])
                if m['name'].lower()=='height':
                    y=float(m['value'].replace('m','')) 
...
###NAME    
    ...
    if assetsize[0]+assetsize[1]+assetsize[2]>0:
        try:
            node.parmTuple('size').set(assetsize)
        except:
            pass
    else:
        try:
            node.parmTuple('size').set([0,0,0])
        except:
            pass        
...
def loadassetLop(node):
    try:    
        ...
        ####AUTOCONVERT
        try:
            auto=hou.getenv("MS_AUTOSHADING")
            if int(auto)==1 and shader == None:
                node.parm('convertshader').pressButton()
        except:
            pass
...
'vray':'vray_vop_material','redshift':'redshift_vopnet'}        
    ...
    matnet=node.node(nd)
    #############Cleanup lop shaders inside
#    if context == 'Lop':
#        if matnet.path()==node.path()+'/shaders':
#            children=matnet.allSubChildren()
...
def packedfromgeo(node):
    if categorytype(node)!=2:
        ...
        context=node.type().category().name() 
        if context=="Lop":
            pythonnode=hou.node(node.path()+'/geometry/python1')
            pfile=hou.node(node.path()+'/out').parm('lopoutput').eval()
        else:
